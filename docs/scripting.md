...
- 编译成功后仍采用事务式初始化：新版本在私有注册键下执行顶层代码和 `pb_init()`，初始化
  期间不进入事件派发或原生兴趣/策略快照，旧版本的已发布行为和原生租约保持不变。新版本
  成功并通过整组原生策略校验后，才调用旧版 `on_unload` 并一次提交；初始化异常或策略冲突
  会撤销新版本刚申请的事件、决定、断点、Hook 和策略，然后恢复旧版继续运行。
...
- **任意时刻热加载/卸载**，包括目标停在断点上的时刻；同名加载 = 替换（先调旧插件的
  `on_unload` 再退役），且只有新版本初始化成功后才正式提交替换。
