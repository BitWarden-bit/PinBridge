...
PbStatus PbBackendAddRegisterEmulator(const PbDebuggerRegDescription*r,uint32_t n,PbGetEmulatedRegisterCallback g,PbSetEmulatedRegisterCallback s,PbGetTargetDescriptionCallback d,void*u){if(Jit()!=PB_OK)return Jit();g_reg_names.clear();g_reg_desc.clear();g_reg_names.reserve(n);g_reg_desc.reserve(n);for(uint32_t i=0;i<n;i++)g_reg_names.push_back(r[i].name);for(uint32_t i=0;i<n;i++){REG native; if(!PbPinRegFromId(r[i].pin_reg,&native)) return PB_ERR_INVALID_ARGUMENT; DEBUGGER_REG_DESCRIPTION x={native,r[i].tool_reg_id,r[i].width_in_bits,g_reg_names[i].c_str(),r[i].gcc_id};g_reg_desc.push_back(x);}g_get=g;g_set=s;g_desc=d;g_emu_user=u;PIN_AddDebuggerRegisterEmulator(n,&g_reg_desc[0],GetThunk,SetThunk,DescThunk,0);return PB_OK;}
