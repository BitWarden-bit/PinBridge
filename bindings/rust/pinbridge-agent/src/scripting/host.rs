...
pub fn publish_decision_interests() {
    let interests = with_registry(|registry| {
        ...
        for plugin in registry.values() {
            ...
            for subscription in plugin.decisions.values() {
                let Some(address) = subscription.address else {
                    continue;
                };
                match subscription.selector {
                    DecisionSelector::HookEntry => interests.push((address, false)),
                    DecisionSelector::HookReturn => interests.push((address, true)),
    ...
    crate::sync_intercept::publish_hook_interests(&interests);
}
...
fn parse_hook_response(
    value: &Bound<'_, PyAny>,
    request: &crate::sync_intercept::HookRequest,
) -> Result<crate::sync_intercept::HookResponse, String> {
    let mut response = crate::sync_intercept::HookResponse::EMPTY;
    if value.is_none() {
        return Ok(response);
    }
    let dict = value
...
fn merge_hook_response(
    aggregate: &mut crate::sync_intercept::HookResponse,
    response: &crate::sync_intercept::HookResponse,
) -> Result<(), String> {
...
fn dispatch_hook_decision() {
    ...
    Python::with_gil(|py| {
        ...
        let mut aggregate = crate::sync_intercept::HookResponse::EMPTY;
        let mut valid = true;
        ...
        if !valid {
            aggregate = crate::sync_intercept::HookResponse::EMPTY;
        }
