...
/// Publishes the exact Hook addresses/phases that have a live synchronous
/// Python interceptor. Native analysis callbacks read only the resulting
/// immutable snapshot and never touch the Python registry.
pub fn publish_decision_interests() {
    let (hooks, entry_all, entry_numbers, exit_all, exit_numbers, exception_all, exception_codes) = with_registry(|registry| {
...
fn extract_word(value: &Bound<'_, PyAny>, field: &str) -> Result<u64, String> {
    value
        .extract::<u64>()
    ...
    if let Some(action) = dict.get_item("action").map_err(|error| error.to_string())? {
        ...
        response.action = match action.trim().to_ascii_lowercase().as_str() {
            "continue" | "proceed" | "resume" => {
                crate::sync_intercept::HOOK_ACTION_CONTINUE
            }
        ...
        .map_err(|error| error.to_string())?
    {
        ...
        for (name, value) in registers.iter() {
            ...
            let Some(index) = crate::arch::gp_registers()
                ...
                return Err(format!("unknown register for {}: {name}", crate::arch::name()));
            };
...
fn dispatch_hook_decision(request: crate::sync_intercept::InterceptRequest) {
    Python::with_gil(|py| {
        ...
        let mut handlers = with_registry(|registry| {
            ...
            for (name, plugin) in registry {
                ...
                for (id, subscription) in &plugin.decisions {
                    ...
                    handlers.push(HookDecisionHandler {
                        plugin: name.clone(),
                        id: *id,
                        callback: subscription.callback.clone_ref(py),
                        once: subscription.once,
        ...
        for handler in handlers {
            ...
            let result = with_plugin_context(&handler.plugin, || {
                ...
                handler
                    ...
                    .call1(py, (event,))
                    .map_err(|error| error.to_string())
                    .and_then(|value| parse_hook_response(value.bind(py), &request))
                    .and_then(|response| {
                        merge_hook_response(&mut aggregate, &response).map(|_| response)
                    })
            });
            ...
            if released && super::decisions::release_hook(handler.address) {
                super::decisions::queue_hook_removal(handler.address);
            }
...
struct SynchronousDecisionHandler {
    plugin: String,
    id: u64,
    callback: Py<PyAny>,
    once: bool,
...
fn dispatch_syscall_decision(request: crate::sync_intercept::InterceptRequest) {
    Python::with_gil(|py| {
        ...
        let mut handlers = with_registry(|registry| {
            ...
            for (name, plugin) in registry {
                ...
                for (id, subscription) in &plugin.decisions {
                    ...
                    handlers.push(SynchronousDecisionHandler {
                        plugin: name.clone(),
                        id: *id,
                        callback: subscription.callback.clone_ref(py),
                        once: subscription.once,
        ...
        for handler in handlers {
            ...
            let result = with_plugin_context(&handler.plugin, || {
                ...
                handler
                    ...
                    .call1(py, (event,))
                    .map_err(|error| error.to_string())
                    .and_then(|value| parse_syscall_response(value.bind(py), &request))
                    .and_then(|response| {
                        merge_syscall_response(&mut aggregate, &response).map(|_| response)
                    })
            });
            let error = result.err();
            if error.is_some() {
...
fn dispatch_exception_decision(request: crate::sync_intercept::InterceptRequest) {
    Python::with_gil(|py| {
        let mut handlers = with_registry(|registry| {
            let mut handlers = Vec::new();
            for (name, plugin) in registry {
        ...
        for handler in handlers {
            ...
            let result = with_plugin_context(&handler.plugin, || {
                ...
                handler
                    ...
                    .call1(py, (event,))
                    .map_err(|error| error.to_string())
                    .and_then(|value| parse_exception_response(value.bind(py)))
                    .and_then(|response| {
                        merge_register_response(&mut aggregate, &response).map(|_| response)
                    })
            });
            let error = result.err();
            if error.is_some() {
...
fn dispatch_sync_decision() {
    ...
    match request.kind {
        crate::sync_intercept::HOOK_ENTRY | crate::sync_intercept::HOOK_RETURN => {
            dispatch_hook_decision(request)
        }
        crate::sync_intercept::SYSCALL_ENTRY | crate::sync_intercept::SYSCALL_EXIT => {
            dispatch_syscall_decision(request)
        }
        crate::sync_intercept::EXCEPTION_HANDLE => dispatch_exception_decision(request),
