...
// PINBRIDGE PATCH (vendored pyo3 0.22.6 for pinbridge-agent):
// Intel Pin maps the agent DLL privately, so the module's TLS index is never
// assigned by the OS loader — std thread_local reads/writes in this process
// alias foreign slots or fault (observed: AccessError in LocalKey::with,
// garbage negative GIL_COUNT tripping LockGIL::bail, write-to-0 on
// std::thread startup). All interpreter access in the agent is
// single-threaded (the scripting thread), so the per-thread GIL refcount
// becomes a process-global atomic: identical semantics for that usage, and
// decref-inlining keeps working because the count only ever changes while
// the GIL is genuinely held.
static GIL_COUNT: AtomicIsize = AtomicIsize::new(0);
...
fn gil_is_acquired() -> bool {
    GIL_COUNT.load(Ordering::Acquire) > 0
}
...
impl SuspendGIL {
    pub(crate) unsafe fn new() -> Self {
        let count = GIL_COUNT.swap(0, Ordering::AcqRel);
...
impl Drop for SuspendGIL {
    fn drop(&mut self) {
        GIL_COUNT.store(self.count, Ordering::Release);
        unsafe {
...
impl LockGIL {
    ...
    fn new(reason: isize) -> Self {
        let count = GIL_COUNT.swap(reason, Ordering::AcqRel);
...
fn increment_gil_count() {
    let current = GIL_COUNT.fetch_add(1, Ordering::AcqRel);
    if current < 0 {
...
fn decrement_gil_count() {
    let current = GIL_COUNT.fetch_sub(1, Ordering::AcqRel);
    debug_assert!(
        current > 0,
