...
$target = Join-Path $dir "code_fetch_python_demo_x64.exe"
$plugin = Join-Path $dir "fetch_policy.py"
...
$ready = Join-Path $dir "code_fetch_warm.ready"
...
$script:Port = $port
$script:Cli = $cli
$log = Join-Path $dir ("code_fetch_python_{0}.agent.log" -f $port)
...
try {
    ...
    if (-not (Test-Path -LiteralPath $ready -PathType Leaf)) {
        throw "target did not confirm the warm call"
    }
    $loaded = $false
    ...
    if (-not $captured.Contains("CODE_FETCH_POLICY_READY")) {
        throw "missing code fetch policy marker: exit=$($pinProcess.ExitCode) stdout=$targetOutput stderr=$targetError log=$captured"
    }
    if ($captured.Contains("callback failed") -or $captured.Contains("top-level failed")) {
        throw "code fetch plugin failed"
    }
    ...
    [ordered]@{
        result = "CODE_FETCH_PYTHON_POLICY_PASS"
        target_exit = $pinProcess.ExitCode
        dynamic_refetch = $true
