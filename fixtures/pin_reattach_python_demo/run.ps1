...
$target = Join-Path $dir "pin_reattach_python_demo_x64.exe"
$plugin = Join-Path $dir "reattach_policy.py"
...
$script:Port = $port
$script:Cli = $cli
$log = Join-Path $dir ("pin_reattach_{0}.agent.log" -f $port)
...
try {
    ...
    if (-not $captured.Contains("REATTACH_READY")) { throw "missing callback marker: REATTACH_READY" }
    $unsupported = $captured.Contains("REATTACH_UNSUPPORTED_SAFE")
    $required = if ($unsupported) { @("REATTACH_UNSUPPORTED_SAFE") } else { @(
        "REATTACH_DETACH_REQUESTED", "REATTACH_DETACHED", "REATTACH_REQUEST_ACCEPTED",
        "REATTACH_ATTACHED", "REATTACH_INSTRUCTION_AFTER_ATTACH", "REATTACH_THREAD_AFTER_ATTACH"
    ) }
    foreach ($marker in $required) {
        if (-not $captured.Contains($marker)) { throw "missing callback marker: $marker" }
    }
    if ($captured.Contains("callback failed") -or $captured.Contains("Pin reattach failed")) {
        throw "reattach callback reported an error"
    }
    $targetOutput = $stdoutTask.Result.Trim()
    $targetError = $stderrTask.Result.Trim()
    if ($pinProcess.ExitCode -ne 0 -or $targetOutput -notmatch "pin_reattach_python_demo") {
        throw "target failed: exit=$($pinProcess.ExitCode) stdout=$targetOutput stderr=$targetError"
    }
    [ordered]@{
        result = if ($unsupported) { "PIN_REATTACH_UNSUPPORTED_SAFE" } else { "PIN_REATTACH_PYTHON_PASS" }
        target_exit = $pinProcess.ExitCode
        reattach_supported = -not $unsupported
        detach_event = -not $unsupported
        attach_event = -not $unsupported
        instruction_after_attach = -not $unsupported
        thread_after_attach = -not $unsupported
